...
class ValueAssessment:
    ...
    def _evaluate_value_alignment(
        ...
        """
        가치와 결정의 정렬도 평가 (고도화 v2.0)
        - 키워드 가중치 적용
        - 구문 근처 분석 (phrase proximity)
        - 문맥 고려
        """
        ...
        positive_keywords = {
            "scientific_integrity": [
                ("truth", 0.4), ("accurate", 0.4), ("rigorous", 0.3),
                ("honest", 0.3), ("verify", 0.35), ("validation", 0.4),
                ("reproducible", 0.4), ("evidence-based", 0.45)
            ],
            "human_benefit": [
                ("help", 0.3), ("improve", 0.35), ("benefit", 0.4),
                ("healthy", 0.35), ("safe", 0.4), ("wellbeing", 0.4),
                ("healing", 0.45), ("support", 0.3)
            ],
            "breakthrough_achievement": [
                ("novel", 0.45), ("innovation", 0.4), ("discover", 0.4),
                ("first", 0.35), ("breakthrough", 0.5), ("revolutionary", 0.45),
                ("groundbreaking", 0.45), ("pioneering", 0.4)
            ],
            "rigorous_methodology": [
                ("test", 0.3), ("experiment", 0.35), ("validate", 0.4),
                ("control", 0.35), ("replicate", 0.4), ("peer-review", 0.45),
                ("systematic", 0.35), ("careful", 0.3)
            ],
            "financial_integrity": [
                ("transparent", 0.4), ("honest", 0.35), ("compliant", 0.35),
                ("ethical", 0.4), ("fair", 0.35), ("legitimate", 0.35),
                ("disclosed", 0.4), ("audited", 0.4)
            ],
            "wealth_creation": [
                ("profit", 0.3), ("return", 0.35), ("gain", 0.3),
                ("growth", 0.35), ("success", 0.3), ("value-creation", 0.4),
                ("yield", 0.35), ("appreciation", 0.35)
            ],
            "artistic_expression": [
                ("creative", 0.4), ("original", 0.45), ("authentic", 0.4),
                ("unique", 0.4), ("express", 0.35), ("vision", 0.4),
                ("innovative", 0.4), ("authentic voice", 0.45)
            ],
            "truth_telling": [
                ("honest", 0.4), ("true", 0.3), ("authentic", 0.4),
                ("real", 0.3), ("genuine", 0.35), ("sincere", 0.35),
                ("truthful", 0.45), ("transparent", 0.4)
            ],
        }
        ...
        negative_keywords = {
            "scientific_integrity": [
                ("falsify", 0.6), ("manipulate", 0.55), ("bias", 0.4),
                ("hide", 0.45), ("lie", 0.55), ("fabricate", 0.65),
                ("misrepresent", 0.6), ("suppress", 0.5)
            ],
            "human_benefit": [
                ("harm", 0.6), ("damage", 0.55), ("risk", 0.35),
                ("danger", 0.5), ("hurt", 0.55), ("endanger", 0.6),
                ("toxic", 0.5), ("exploit", 0.6)
            ],
            "breakthrough_achievement": [
                ("derivative", 0.45), ("copy", 0.5), ("incremental", 0.3),
                ("trivial", 0.35), ("plagiarism", 0.65), ("imitate", 0.5),
                ("mundane", 0.3), ("banal", 0.35)
            ],
            "rigorous_methodology": [
                ("rush", 0.45), ("skip", 0.5), ("shortcut", 0.5),
                ("untested", 0.45), ("guess", 0.4), ("sloppy", 0.5),
                ("negligent", 0.55), ("careless", 0.5)
            ],
            "financial_integrity": [
                ("fraud", 0.7), ("deception", 0.65), ("insider", 0.55),
                ("manipulation", 0.65), ("illegal", 0.7), ("embezzle", 0.7),
                ("money laundering", 0.75), ("bribery", 0.7)
            ],
            "wealth_creation": [
                ("loss", 0.3), ("fail", 0.35), ("bankrupt", 0.5),
                ("collapse", 0.45), ("crash", 0.4), ("default", 0.45)
            ],
            "artistic_expression": [
                ("constrain", 0.4), ("censor", 0.55), ("limit", 0.35),
                ("restrict", 0.45), ("control", 0.4), ("suppress", 0.5),
                ("prohibit", 0.4), ("forbid", 0.4)
            ],
            "truth_telling": [
                ("lie", 0.6), ("deceive", 0.65), ("fake", 0.5),
                ("false", 0.45), ("pretend", 0.4), ("mislead", 0.6),
                ("distort", 0.55), ("exaggerate", 0.4)
            ],
        }
        ...
        if value_name in positive_keywords:
            for keyword, weight in positive_keywords[value_name]:
                if keyword in decision_lower:
                    influence += weight * 0.4  # 기본 0.3 → 0.4로 강화
                    if len(reason_parts) < 2:
                        reason_parts.append(f"✓ {keyword}")

        # 부정 키워드 확인 (더 높은 페널티)
        if value_name in negative_keywords:
            for keyword, weight in negative_keywords[value_name]:
                if keyword in decision_lower:
                    influence -= weight * 0.5  # 기본 0.4 → 0.5로 강화
                    if len(reason_parts) < 2:
                        reason_parts.append(f"✗ {keyword}")
        ...
        return ValueInfluence(
            ...
            reason=" | ".join(reason_parts) if reason_parts else "Neutral alignment"
        )
